...
export async function loadTemplateComponents(content) {
      let __awaits = [];
      let usedCEL = new Set(), usedCE = new Set(), forms = new Set();
      let tw = document.createTreeWalker(content, NodeFilter.SHOW_ELEMENT ,
      ...
      return Promise.all( [...forms].map(c => customLoader?.(c))).then( () => {
            __awaits.push(...([...forms].map(x => loadTemplateComponents(customElements.get(x).template.tpl.content))));
            let tw = document.createTreeWalker(content, NodeFilter.SHOW_COMMENT,
                { acceptNode: n => n._tpl ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP } );
            while( tw.nextNode() ) {
                  let node = tw.currentNode;
                  console.log(node)
            }
